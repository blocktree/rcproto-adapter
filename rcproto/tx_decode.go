...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.CreateRCPRawTransaction(wrapper, rawTx)
}

//SignRawTransaction 签名交易单
func (decoder *TransactionDecoder) SignRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.SignRCPRawTransaction(wrapper, rawTx)
}

//VerifyRawTransaction 验证交易单，验证交易单并返回加入签名后的交易单
func (decoder *TransactionDecoder) VerifyRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.VerifyRCPRawTransaction(wrapper, rawTx)
}
...
func (decoder *TransactionDecoder) CreateRCPRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if !isActived && amount.Cmp(big.NewInt(decoder.wm.Config.ReserveAmount)) < 0 {
		return errors.New("The destination address [" + to + "] is not actived yet, if you owned the address, you need to send at least 20 RCP to active it, these 20 RCP will be locked forever!")
	}
	...
	for _, a := range addressesBalanceList {
		...
		if a.Balance.Cmp(amount) < 0 {
			...
			if count.Cmp(amount) >= 0 {
				...
				decoder.wm.Log.Std.Notice("The RCP of the account is enough," +
...
func (decoder *TransactionDecoder) SignRCPRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
...
func (decoder *TransactionDecoder) VerifyRCPRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
...
func (decoder *TransactionDecoder) CreateSimpleSummaryRawTransaction(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransaction, error) {
	...
	for _, addrBalance := range addrBalanceArray {
		...
		if !isActived && sumAmount_BI.Cmp(big.NewInt(decoder.wm.Config.ReserveAmount)) < 0 {
			return nil, errors.New("The summary address [" + sumRawTx.SummaryAddress + "] is not actived yet, if you owned the address, you need to send at least 20 RCP to the address to active it, these 20 RCP will be locked forever!")
		}
